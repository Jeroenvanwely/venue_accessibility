        ...
        for i in range(len(x_borders)):
            for j in range(len(y_borders)):
                ...
                if len(pt_idx) > 3:
                    ...
                    # Compute average minimum and maximum height of points within cell.          
                    z_values = np.sort(z[pt_idx])
                    z_min = np.mean(z_values[0:3])

                    condition = ((z_values - z_min > max_h_dif) & (z_values - z_min < 1.8))
                    wrong_pt_count = len(np.where(condition)[0])

                    z_values = np.sort(z_values[np.where((z_values - z_min < max_h_dif))[0]])
                    z_min, z_max = np.mean(z_values[0:3]), np.mean(z_values[-3:])
                    z_differ = z_max - z_min

                    if wrong_pt_count < 3 and z_differ >= min_h_dif:
                        idx_to_return += list(pt_idx)
...
def find_potential_step_points2(pts, poi_idx, params):
    """
    Function to filter out small areas from given set of connected components.
        ...
        for i in range(len(x_borders)):
            ...
            for j in range(len(y_borders)):
                ...
                if len(pt_idx) > 15:
                    ...
                    correct_height_jump = np.where((z_values - z_min < max_h_dif))[0]
                    z_values = np.sort(z_values[correct_height_jump])
                    z_min, z_max = np.mean(z_values[0:3]), np.mean(z_values[-3:])
                    z_differ = z_max - z_min

                    if wrong_pt_count < 3 and z_differ >= min_h_dif:
                        idx_to_return += list(pt_idx[correct_height_jump])
                        new_cell = [z_max, z_min, i, j, list(pt_idx[correct_height_jump])]
                        to_return.append(new_cell)

        
    potential_step_pts = poi_idx[idx_to_return]
    return np.unique(potential_step_pts), to_return